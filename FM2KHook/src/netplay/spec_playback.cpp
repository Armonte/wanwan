...
// ApplyResetInputState) and PopFrameInputs (the frame pacemaker + the snapshot,
// battle-align and deep-join holds + the natural-boot walk + the pop itself).
// Extracted VERBATIM from spectator_node.cpp. Apply fns live in namespace
// specnode (also called by snapshot-cache); the rest is public API (decls in
// spectator_node.h).
//
// Sibling TUs, all sharing spectator_node_internal.h's g_state:
            ...
            // results screen after the Phase F boundary rework).
            {
                ...
                if (s_live_spec == 1) {
                    ...
                    if (g_state.pb_boundary == State::PbBoundary::NONE) {
                        ...
                        SeamResetWalkState();   // walk counters live in
                                                // spec_playback_seam.cpp
                    }
                    g_state.pb_boundary = State::PbBoundary::SEAM;
...
bool SpectatorNode_PopFrameInputs(uint16_t* p1_input, uint16_t* p2_input) {
    ...
    // Match-boundary state machine (spec_playback_seam.cpp). Moved out of this
    // function VERBATIM when this TU hit the 1000-line cap; the three outcomes
    // are exactly the three the inline block had.
    //   FALL_THROUGH -- no boundary active, or it released into normal playback
    //                   on this tick: continue to the pop below.
    //   FEED         -- the seam drove the sim on synthetic input and already
    //                   wrote pb_current_p1/p2 + the out-params.
    //   HOLD         -- nothing may advance this tick.
    switch (SeamStep(p1_input, p2_input)) {
        case SeamStepResult::HOLD:         return false;
        case SeamStepResult::FEED:         return true;
        case SeamStepResult::FALL_THROUGH: break;
    }
