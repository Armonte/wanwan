...
namespace Gekko {
	...
	public:
        ...
        // history_size: caller's GekkoConfig::input_history_size. Pass 0
        // to keep the legacy 128-frame cap (pre-late-joiner behavior).
        void Init(u8 num_players, u32 input_size, u32 history_size);
	...
	private:
		// Default input-queue cap when GekkoConfig::input_history_size is 0.
		// Bumped (via the config field) by callers that want late-joining
		// spectators to receive a full session backfill on connect — see
		// README.md:36 (Work in progress).
		const u32 DEFAULT_MAX_INPUT_QUEUE_SIZE = 128;
		u32 _max_input_queue_size = DEFAULT_MAX_INPUT_QUEUE_SIZE;
