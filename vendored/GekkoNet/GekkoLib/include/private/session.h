...
struct GekkoSession {
    ...
    // Highest frame for which inputs from ALL players have actually been
    // received (no predictions involved). Local FM2K patch: lets the host
    // recorder/spectator stream emit only confirmed inputs instead of
    // speculative ones. NULL_FRAME(-1) until first inputs arrive.
    virtual i32 ConfirmedFrame() = 0;
