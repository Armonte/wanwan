...
void Gekko::MessageSystem::Init(u8 num_players, u32 input_size, u32 history_size)
{
    ...
    // (last_acked_frame == NULL_FRAME) can receive everything from frame
    // 0 via the existing SendInputsToPeer math (peer_start_frame =
    // max(last_acked + 1, queue_oldest_frame)). 0 keeps the legacy cap.
    if (history_size > 0) {
        _max_input_queue_size = history_size;
    }
...
void Gekko::MessageSystem::AddInput(Frame input_frame, Handle player, u8 input[], bool remote)
{
    ...
    input_q.TrimToAck(min_ack, _max_input_queue_size);
}
...
void Gekko::MessageSystem::AddSpectatorInput(Frame input_frame, u8 input[])
{
    ...
    input_q.TrimToAck(GetMinLastAckedFrame(true), _max_input_queue_size);
}
...
bool Gekko::MessageSystem::CheckStatusActors()
{
    ...
    for (u32 i = 0; i < 2; i++)
    {
        const bool is_spectator_pass = (i == 1);
            ...
            if (player->GetStatus() == Initiating) {
                if (player->stats.last_sent_sync_message + NetStats::SYNC_MSG_DELAY < now) {
                    if (player->sync_num == 0) {
                        ...
                        // Spectators don't increment result either: their
                        // connect status doesn't gate session advance, and
                ...
                // can connect when they catch up, but a slow / never-arriving
...
void Gekko::MessageSystem::ParsePacket(NetAddress& addr, NetPacket& pkt, u32 packet_size)
{
    ...
    if (pkt.header.magic != _session_magic) {
        ...
        // "dropped packet!" here, which spammed the console during every
        // session start and confused users into thinking packets were
        // being lost. Re-enable for diagnostics by reverting this hunk.
    }
    else {
