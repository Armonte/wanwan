...
void Gekko::SpectatorSession::Init(GekkoConfig* config)
{
    ...
    // When input_history_size is set (host-side backfill enabled), the
    // spectator's receive buffer must be at least that large to hold a
    // late-joiner's catch-up burst from the host. See README.md:36.
    u32 buffer_size = InputBuffer::DEFAULT_BUFF_SIZE + _config.spectator_delay;
    if (_config.input_history_size > buffer_size) {
        buffer_size = _config.input_history_size;
    }
    _sync.Init(_config.num_players, _config.input_size, buffer_size);

    // setup message system.
    _msg.Init(_config.num_players, _config.input_size, _config.input_history_size);
...
i32 Gekko::SpectatorSession::ConfirmedFrame()
{
